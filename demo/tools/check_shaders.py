...
# C++ name -> JavaScript name. kOilPreamble/kOilLibrary/kOilMain are the three
# pieces OilShaderSource() concatenates, and kSimmerPreamble/kOilLibrary/
# kSimmerMain the three SimmerShaderSource() does; the demo concatenates the
# same pieces in the same order, so they are checked individually rather than
# as one assembled string -- a drift is then attributed to the piece it is in
# rather than to "the oil pass".
#
# kOilLibrary is the one that matters most. It is the per-pixel half of
# Oil.cpp transcribed into GLSL, so it is ALREADY a second copy of something;
# this page makes it a third, and only this check keeps all three in step.
#
# kProbePreamble and kProbeMain are deliberately absent: the probe exists only
# for `fltest --field` and has no counterpart on the page.
PASSES = {
    "kVertexShader": "VERTEX_SHADER",
    "kOilLibrary": "OIL_LIBRARY",
    "kOilPreamble": "OIL_PREAMBLE",
    "kOilMain": "OIL_MAIN",
    "kSimmerPreamble": "SIMMER_PREAMBLE",
    "kSimmerMain": "SIMMER_MAIN",
...
def unescape(source):
    """Undo the escapes a template literal forces, and nothing else.

    Order matters: the backslash escape has to come last, or unescaping a
    backtick would leave a lone backslash that the next replacement eats.
    """
    return (
        source.replace("\\`", "`")
        .replace("\\${", "${")
